...
const ExpertUploadExperience: React.FC<{
  lang: Language;
  t: any;
  toggleLanguage: () => void;
  ...
  // ✅ بدل ما يروح login، يفتح Coming Soon
  const handleProceed = () => {
      ...
      <div className="pt-28">
        <div className="max-w-7xl mx-auto px-6 pt-14 pb-24">
          ...
          <div className="text-center mb-14">
            <h1 className="text-4xl md:text-5xl font-black mb-4">{t.expertUpload.title}</h1>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {["s1", "s2", "s3"].map((k, idx) => (
                <div
                  key={k}
                  className="bg-[#0A0A0A]/40 border border-[#1E3A75] rounded-2xl p-6"
                >
                  <p className="text-xs uppercase tracking-[0.2em] text-[#F4F3EC]/40 font-bold mb-2">
                    {lang === "en" ? `Step 0${idx + 1}` : `الخطوة 0${idx + 1}`}
                  </p>
                  <p className="text-[#F4F3EC] font-bold">
                    {t.expertUpload.steps[`${k}Title`]}
                  </p>
                  <p className="text-sm text-[#F4F3EC]/60 mt-2">
                    {t.expertUpload.steps[`${k}Desc`]}
                  </p>
                </div>
              ))}
            </div>
